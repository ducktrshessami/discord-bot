...
class Response { // Bot response class
    ...
    async check(message, execute = true) { // Check if a message would trigger this response
        if (this._listCheck(message) && await this.checkFunction(message, this.trigger)) {
            if (execute) {
                this.say(message);
            }
            return true;
    ...
    _listCheck(message) { // Check if the right conditions are met
        ...
        if (this.serverWhitelist.length) { // Check server lists
            ...
            server = !this.serverBlacklist.includes(message.guild.id);
        }
        if (this.channelWhitelist.length) { // Check channel lists
            ...
            channel = !this.channelBlacklist.includes(message.channel.id);
        }
