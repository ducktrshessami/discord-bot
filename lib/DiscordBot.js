...
function defaultHelpCmd(message, args) { // Pregenerated help command
    ...
    if (args.length > 1) {
        ...
        if (match && (!match.hidden || this.client.config.botmin.includes(message.author.id))) {
...
class DiscordBot extends discord.Client { // Main bot class
    ...
    _handleCommands(message) { // Handle commands
        ...
        if (cPrefix = this._findPrefix(message)) {
            return this.commands.some(cmd => cmd.check(message, cPrefix, this.config.botmin.includes(message.author.id)));
        }
        return false;
    }
