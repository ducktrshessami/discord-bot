...
function defaultPrefixCmd(message, args) { // Pregenerated prefix command
    if (args.length > 1 && message.member.hasPermission("ADMINISTRATOR")) {
        ...
        this.client.emit("configUpdate", this.client.config);
...
function defaultHelpCmd(message, args) { // Pregenerated help command
    ...
    if (args.length > 1) {
        ...
        if (match && (!match.hidden || this.client.config.botmins.includes(message.author.id))) {
...
function generateCmdListEmbed(commands, color = "RANDOM") { // Pregenerated command list for help command
    return new discord.MessageEmbed({
        ...
        description: commands
            .sort()
            .filter(cmd => !cmd.botmin && !cmd.hidden)
...
class DiscordBot extends discord.Client { // Main bot class
    constructor(config = {}, commands = [], responses = []) {
        ...
        this.on("ready", () => {
            ...
            this.guilds.cache.each(this._handleGuildConfigInit, this); // Update server names
    ...
    _handleCommands(message) { // Handle commands
        ...
        if (cPrefix = this._findPrefix(message)) {
            return this.commands.some(cmd => cmd.check(message, cPrefix, this.config.botmins.includes(message.author.id)));
        }
        return false;
    }

    async _handleResponses(message) { // Handle responses
        for (let response of this.responses) {
            if (await response.check(message)) {
                return true;
            }
        }
        return false;
    ...
    _handleGuildConfigInit(guild) { // Update server config without emitting event
    ...
    _handleGuildCreate(guild) { // Update server configs
        ...
        if (this.config.servers[guild.id].name !== guild.name) {
            this.config.servers[guild.id].name = guild.name;
            updated = true;
        }
        if (updated) {
    ...
    _generatePrefixCmd() {
        let prefixCmd = new Command("prefix", defaultPrefixCmd, {
            ...
            description: "Set or display the current server's custom command prefix. Requires admin to set",
