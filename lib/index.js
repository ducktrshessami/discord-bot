...
function defaultPrefixCmd(message, args) { // Pregenerated prefix command
    if (args.length > 1) {
        this.config.servers[message.guild.id].prefix = args[1];
        message.channel.send(`Custom prefix set to \`${args[1]}\``).catch(this.client._emitError);
    }
    else {
        if (this.config.servers[message.guild.id].prefix) {
            message.channel.send(`Current custom prefix: \`${this.config.servers[message.guild.id].prefix}\``).catch(this.client._emitError);
        }
        else {
            message.channel.send("Custom prefix not set").catch(this.client._emitError);
        }
    }
}
...
function defaultHelpCmd(message, args) { // Pregenerated help command
    let handled = false;
    if (args.length > 1) {
        let match = this.client.commands.find(cmd => cmd.name == args[1].toLowerCase());
        if (match && (!match.hidden || this.client.config.admin.includes(message.author.id))) {
            handled = true;
            message.channel.send(`\`${match.usage}\`
${match.description}
${match.subtitle}`.trim()).catch(this.client._emitError);
        }
    }
    if (!handled) {
        message.channel.send("", this.client.helpEmbed).catch(this.client._emitError);
    }
}
